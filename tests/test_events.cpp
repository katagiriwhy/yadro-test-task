...
TEST(EventTest, IOTest) {
    ...
    std::ifstream file(std::string(TEST_DATA_DIR) + "/events.txt");
    ASSERT_TRUE(file.is_open()) << "Cannot open file";

    auto event = std::make_unique< ClientEvent >();
    file >> event;
    EXPECT_EQ(typeid(*event), typeid(ClientEnteredEvent));
    ...
    testing::internal::CaptureStdout();
    std::cout << ErrorEvent(Time({9, 9}), ErrorType::NotOpenYet);
    output = testing::internal::GetCapturedStdout();
